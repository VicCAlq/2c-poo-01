...
const livroReceitas = {
  ...
  buscarReceitaPorNome: function(nome) {
    for (let i = 0; i < this.receitas.length; i++) {
      if (this.receitas[i].nome === nome) {
        return this.receitas[i]
      }
    }
    return null
  },
  ...
  listarTodasReceitas: function() {
    const nomes = []
    for (let i = 0; i < this.receitas.length; i++) {
      nomes.push(this.receitas[i].nome)
    }
    return nomes
  },
  ...
  filtrarPorIngrediente: function(ingrediente) {
    const resultado = []
    for (let i = 0; i < this.receitas.length; i++) {
      if (this.receitas[i].ingredientes.includes(ingrediente)) {
        resultado.push(this.receitas[i])
      }
    }
    return resultado
  }
...
const cozinheiro = {
  ...
  criarReceita: function(nome, listaIngredientes, tempo) {
    const ingredientesValidos = []

    for (let i = 0; i < listaIngredientes.length; i++) {
      if (ingredientes.includes(listaIngredientes[i])) {
        ingredientesValidos.push(listaIngredientes[i])
      }
    }
  ...
  verificarPossibilidade: function(disponiveis) {
    ...
    for (let i = 0; i < this.receitasCriadas.length; i++) {
      let pode = true

      for (let j = 0; j < this.receitasCriadas[i].ingredientes.length; j++) {
        if (!disponiveis.includes(this.receitasCriadas[i].ingredientes[j])) {
          pode = false
        }
      }

      if (pode) {
        possiveis.push(this.receitasCriadas[i].nome)
      }
    }
